...
import { transportationGoals } from "@metadata/goals";
...
const Transportation = ({ isQuarterlyPlan }) => {
  ...
  const fetchPlanDetails = async () => {
    try {
      ...
      if (transportationData) {
        transportationData = cloneDeep(transportationData);
        const transportationEntries = transportationData?.transportationEntries;
        ...
        if (!!transportationEntries?.length) {
          entries = transportationEntries?.map(entry => {
            ...
            const goal = transportationGoals?.find(
              ({ value }) => type === value
            );
        ...
        setAnnualGoal(transportationData?.annualGoal);
      }
  ...
  const onSave = async () => {
    try {
      ...
      const payload = {
        ...
        goal: { transportation: { annualGoal } },
      };
    ...
    <FlexBox column width="100%" rowGap="1.5rem">
      ...
      <FlexBox column rowGap="0.75rem" align="flex-end">
        <EntryTable
          ...
          goalOptions={transportationGoals}
